...
1. **데이터 전처리**
    ...
    a. 결측치 처리 및 데이터 클리닝  
        i. 데이터셋에 존재하는 많은 결측치를 서울시 건축물 대장 데이터와 외부 데이터로 채움  
        ii. 사용 라이브러리: pandas, numpy, matplotlib.pyplot, seaborn  
    
    b. 피처 엔지니어링  
        i. EDA를 바탕으로 ['꼭대기층 여부', '이전가격', '전용면적', '아파트 평균높이', '연GDP', '층', '계약년월', 'y', '한강거리', '500m이내 정류장 수', '건물나이'] 와 같은 파생변수 생성  
        ii. 사용 라이브러리: pandas, numpy, LightGBM, Optuna  

2. **모델 구성**

    a. **LightGBM 모델**  
        i. 저가 아파트와 고가 아파트를 예측하는 모델을 분리하여 구성  
        ii. 저가 아파트 모델: 실거래가가 30억 미만인 데이터만을 사용  
        iii. 고가 아파트 모델: 실거래가가 30억 이상인 데이터만을 사용  
        iv. 사용 하이퍼파라미터: Optuna 라이브러리를 이용해 최적의 하이퍼파라미터 탐색  
            - learning_rate: 0.05  
            - num_leaves: 31  
            - max_depth: -1  
...
1. 아파트 실거래가 예측 모델을 LightGBM을 통해 구현  
    a. 저가 아파트 모델링의 경우, 실거래가 예측을 어느 정도 잘 수행  
        i. LightGBM 모델의 최종 Train RMSE: 4958.61  
        ii. LightGBM 모델의 최종 Valid RMSE: 9761.01  
        iii. LightGBM 모델의 최종 Test RMSE: 10015.53  

    b. 고가 아파트 모델링의 경우, 실거래가 예측이 저가 아파트 모델보다 정확도가 낮음  
        i. LightGBM 모델의 최종 Train RMSE: 5493.75  
        ii. LightGBM 모델의 최종 Valid RMSE: 26855.08  
        iii. LightGBM 모델의 최종 Test RMSE: 10015.53  

2. 최종 제출 모델이 Public 및 Private 리더보드에서 좋은 성능을 보임  
    a. 최종 Public RMSE: 104262.69  
    b. 최종 Private RMSE: 86148.23  

3. 모델을 통해 아파트 가격 예측에 도움을 주었지만, 정확도 향상에는 한계가 있었음  
    a. 저가 아파트 모델이 고가 아파트 모델에 비해 예측 정확도가 높음  
    b. 고가 아파트 모델은 고가 아파트 가격의 변동성 때문에 예측에 어려움을 겪음  
    c. 평균적인 가격 예측보다는 극단적인 가격 예측에서 성능 저하가 나타남  
